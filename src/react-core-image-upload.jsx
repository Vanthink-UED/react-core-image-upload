...
class ReactCoreImageUpload extends React.Component {
  ...
  __find(ele) {
    const dq = document.getElementById(this.state.formID);
    return dq.querySelector(ele);
  }

  change(e) {
    const fileVal = this.__find('input').value.replace(/C:\\fakepath\\/i, '');
    ...
    if (e.target.files[0].size > this.props.maxFileSize) {
      let formatSize;
      if (parseInt(this.maxFileSize / 1024 / 1024, 10) > 0) {
        formatSize = (this.maxFileSize / 1024 / 1024).toFixed(2) + 'MB';
      } else if (parseInt(this.maxFileSize / 1024, 10) > 0) {
